...
// Need:
// 1. Fits image class that can do collective MPI-IO read/write -- TODO
// 2. median/quartile determination, selection. excluding NaNs -- avail in casacore
// 3. polynomial fitting & evaluation -- avail in casacore
...
public:
    virtual int run(int argc, char* argv[])
    {
        ...
        try {
            ...
            // get header size, get image dimensions
            casa::Int nx = 1, ny = 1, nz = 1;
            casa::Long headersize= 0;
            ...
            casa::Vector<casa::Float> vec(casa::IPosition(1,bufsize), bufl, casa::SHARE);
        ...
        {
            fitsfile *infptr;  // FITS file pointer
        ...
        void copy_header(const casa::String &infile, const casa::String& outfile, casa::Long headersize)
        {
            ...
            if (file.is_open())
            {
                ...
                file.close();
                ofstream ofile (outfile.c_str(), ios::out|ios::binary|ios::trunc);
                if (ofile.is_open()) {
        ...
        void process_spectrum(casa::Vector<casa::Float>& vec, float threshold, int order) {
            ...
            indgen(x_full);
            x_full -= xmean;
            for (size_t i = 0; i<n; i++) y_full(i) = vec(i) - (offset + slope * x_full(i));
            //y_full = x_full;
            //y_full *= -slope;
            //y_full -= offset;
            //y_full += vec;
